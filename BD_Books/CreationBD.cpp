...
typedef struct {
  int  id;
  int caddy_id; // cle etrangere
  int book_id; // cle etrangere
  int quantite;
} CADDY_ITEMS;
...
int main(int argc,char *argv[])
{
  ...
  if(mysql_query(connexion, "CREATE TABLE EMPLOYES("
                      "id INT(4) AUTO_INCREMENT PRIMARY KEY,"
                      "login VARCHAR(20),"
                      "password VARCHAR(25));")){
  ...
  if(mysql_query(connexion, "CREATE TABLE CADDIES("
                    ...
                    "deja_paye DECIMAL(10,2),"
                    "FOREIGN KEY (client_id) REFERENCES clients(id));")){

  finish_with_error(connexion);
 }

  // Creation table CADDY_ITEMS
